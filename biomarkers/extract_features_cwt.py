...
ROW_LIMIT = -1  # Process the first 4500 records
SAMPLING_RATE = 500
FILTER_LOWCUT = 0.5
FILTER_HIGHCUT = 40.0
OUTPUT_DIR = project_root / "biomarkers"
os.makedirs(OUTPUT_DIR, exist_ok=True)
CSV_OUTPUT_PATH = OUTPUT_DIR / "ecg_biomarkers_full_cwt.csv"
REPORT_OUTPUT_PATH = OUTPUT_DIR / "full_extraction_report_cwt.md"
...
def extract_record_features(record, fs=SAMPLING_RATE):
    ...
    # 3. Delineate subset of leads (II, V5, V1, I) using CWT
    delineations = {}
    for lead in ["II", "V5", "V1", "I"]:
        lead_sig = clean_signals[:, LEADS_MAP[lead]]
        try:
            _, waves = nk.ecg_delineate(lead_sig, ii_r_peaks, sampling_rate=fs, method="cwt")
            
            # Align waves to ii_r_peaks to resolve NeuroKit2 CWT list length shifts
            aligned_waves = {}
            
            # Helper to align marker lists
            def align(marker_key, win_l, win_r):
                markers = waves.get(marker_key, [])
                if hasattr(markers, "tolist"):
                    markers = markers.tolist()
                elif isinstance(markers, np.ndarray):
                    markers = list(markers)
                
                aligned = [np.nan] * len(ii_r_peaks)
                if not markers:
                    return aligned
                valid_m = [m for m in markers if not pd.isna(m)]
                for i, r in enumerate(ii_r_peaks):
                    candidates = [m for m in valid_m if r + win_l <= m <= r + win_r]
                    if candidates:
                        aligned[i] = min(candidates, key=lambda m: abs(m - r))
                return aligned

            # Align all wave markers returned by CWT
            aligned_waves["ECG_P_Onsets"] = align("ECG_P_Onsets", -150, -25)
            aligned_waves["ECG_P_Peaks"] = align("ECG_P_Peaks", -120, -15)
            aligned_waves["ECG_P_Offsets"] = align("ECG_P_Offsets", -100, -10)
            aligned_waves["ECG_Q_Peaks"] = align("ECG_Q_Peaks", -50, 0)
            aligned_waves["ECG_R_Onsets"] = align("ECG_R_Onsets", -50, 0)
            aligned_waves["ECG_R_Offsets"] = align("ECG_R_Offsets", 0, 50)
            aligned_waves["ECG_S_Peaks"] = align("ECG_S_Peaks", 0, 50)
            aligned_waves["ECG_T_Onsets"] = align("ECG_T_Onsets", 30, 150)
            aligned_waves["ECG_T_Peaks"] = align("ECG_T_Peaks", 50, 200)
            aligned_waves["ECG_T_Offsets"] = align("ECG_T_Offsets", 80, 250)
            
            delineations[lead] = aligned_waves
        except Exception as e:
            delineations[lead] = {}
...
def main():
    ...
    parser.add_argument("--log_csv", type=str, default=str(OUTPUT_DIR / "qc_logs_cwt.csv"), help="Output path for QC logs CSV")
